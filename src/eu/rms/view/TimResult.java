...
public class TimResult extends GridPane {
	...
	public TimResult(SearchResult[] searchResults) {
		...
		more.setOnAction(λ -> {
			TimResultOverview detailView = new TimResultOverview("1101");
	...
	private void getStartInfo(SearchResult result) {
		...
		GetDB("SELECT a.timTimeOnDeparture, a.timPlatform, b.staName, c.linType, c.linNr, c.linKey, c.linDestination FROM tStation b JOIN tStationTimetable a ON b.staKey=a.staId JOIN tLine c ON a.linId=c.linKey WHERE b.staName = '"
				+ result.getLinStart() + "' AND c.linKey = '" + result.getLinId() + "';", "start");
	}

	private void getEndInfo(SearchResult result) {

		GetDB("SELECT a.timTimeOnDeparture, a.timTimeOnArrival, a.timPlatform, b.staName, c.linType, c.linNr, c.linKey, c.linDestination FROM tStation b JOIN tStationTimetable a ON b.staKey=a.staId JOIN tLine c ON a.linId=c.linKey WHERE b.staName = '"
				+ result.getLinEnd() + "' AND c.linKey = '" + result.getLinId() + "';", "end");
	}

	public String GetDB(String string, String string2) {
		Connection conn;
		try {
			...
			String arrivalTime = "00:00";// rs.getString("timTimeOnArrival");
	...
	private void addEndInfo(String arrivalTime) {
		this.setDatArrivalTime(arrivalTime);
	}
